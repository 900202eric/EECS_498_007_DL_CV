...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
      dists[i] = torch.sum((x_test_flattened - x_train_flattened[i])**2, dim=1).t()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_test_flattened = x_test.reshape((num_test, -1))
  # (x-y)^2 = x^2 + y^2 - 2xy
  dists = torch.sum(x_test_flattened**2, dim=1) + torch.sum(x_train_flattened**2, dim=1).reshape(-1, 1) - 2 * (x_train_flattened @ x_test_flattened.t())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    # print(dists[:, i])
    values, idxs = torch.topk(dists[:, i], k=k, dim=0, largest=False)
    # print(values, idxs)
    # print(y_train[idxs])
    modeValues, modeIdxs = torch.mode(y_train[idxs]) # 找出眾數
    y_pred[i] = modeValues
    # print(modeValues, modeIdxs)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      k_to_accuracies[k].append(classifier.check_accuracy(val_X, val_Y, k=k, quiet=True))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
